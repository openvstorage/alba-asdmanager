    ...
    def collect(target, revision=None, suffix=None):
        ...
        * Any existing tag for repackging 'alpha', 'beta' or 'release' packages
        ...
        if target in ['test', 'stable', 'release']:
            ...
            changelog.append('ALBA ASD Manager')
            changelog.append('================')
            changelog.append('')
        ...
        SourceCollector._run(
            "tar -czf {0}/alba-asdmanager_{1}.tar.gz --transform 's,^,openvstorage-{1}/,' scripts/install scripts/system config ovs webapps *.txt".format(
